...
namespace PokemonGo.RocketAPI.Helpers
{
    public class S2Helper
    {
        public static List<ulong> GetNearbyCellIds(double longitude, double latitude)
        {
            ...
            (*/
            nearbyCellIds.AddRange(cellId.GetEdgeNeighbors());

            for (int i = 0; i < 3; i++)
            {
                foreach (var entry in nearbyCellIds.ToArray())
                {
                    var otherNearby = entry.GetEdgeNeighbors();
                    nearbyCellIds.AddRange(otherNearby);
                }
            }
            nearbyCellIds = nearbyCellIds.Distinct().ToList();
